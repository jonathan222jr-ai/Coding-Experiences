...
public class Scan {
	...
	private boolean hasError; // indicates if any invalid token was encountered
	...
	/**
	 * Returns true if any invalid token was encountered during scanning
	 * @return boolean
	 */
	...
	public Pair<TokenNames,String> getNextToken() {
		...
		if(ch == '\n' && lastFoundToken == TokenNames.None) {
			...
			currentToken = token.getToken(characters);
			if(currentToken == TokenNames.None) {
				// we have reached the end of the line and found no tokens so mark error and stop scanning
