...
| [`eng-agent-qa`](ai-agents/eng-agent-qa) | A later take on the same system, built around automatic quality gates: every code-generation step is followed by a review agent and then a debugger agent that fixes what the review flags. Adds a `ReportPackager` that reads the finished run report, extracts the code blocks, and writes a runnable zip; run reports are authored by Gemini. |
